...
infile3 = paste0('~/surfdrive/work/allocation/validation/',site_name,'.rwl');
trw.obs = read.rwl(infile3,format='tucson');
...
#-- 1.3 Plot ####
png(paste0('figure/trend_',site,'.png'), width=11.5, height=8.5, 
    units="in", res=300)
par(mfrow=c(2,2))
...
mtext(side=2,'Tree-ring width (mm)',line=2.5,cex=1.5)
mtext(side=1,'Tree age (years)',line=2.5,cex=1.5)

lines(apply(trw.age,1,mean),lty=2,lwd=2)
...
at = seq(1,nyears,by=interval)
axis(1, at=seq(1,nyears,by=interval), labels = at,cex.axis=1.8)
mtext(side=2,'Observation - simulation (mm)',line=2.5,cex=1.5)
mtext(side=1,'Tree age (years)',line=2.5,cex=1.5)
...
png(paste0('figure/mature_',site,'.png'), width=11.5, height=12, units="in", res=300)
par(mfrow=c(3,2))
par(mar=c(5.3,5,4.1,2.1))
...
#-- 3.2 Plot #### 
png(paste0('figure/young_',site,'.png'), width=11.5, height=12, units="in", res=300)
par(mfrow=c(3,2))
par(mar=c(5.3,5.1,4.1,2.1))
...
#-- 4.2 Plot ####
png(paste0('figure/extreme_',site,'.png'), width=11.5, height=12, units="in", res=300)
par(mar=c(5.2,7.5,5.5,2.1))
par(mfrow=c(3,2))
...
mtext(side=2,'Simulated tree from \n the biggest diameter class (mm)',line=3.1,cex=1.3)
mtext(side=1,'Calendar year',line=3.5,cex=1.3)
...
mtext(side=2,'Simulated output at \n extreme years (Normalized)',line=3.1,cex=1.3)
